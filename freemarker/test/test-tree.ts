...
function formatLezerTree(
  tree: ReturnType<typeof parser.parse>,
  source: string,
  options: {
    includeRange?: boolean;
    includeLeafText?: boolean;
    maxLeafTextLength?: number;
  } = {},
) {
  const { includeRange = true, includeLeafText = true, maxLeafTextLength = 40 } =
    options;
  ...
  function walk(depth: number) {
    ...
    if (includeLeafText && cursor.to > cursor.from) {
      const raw = source.slice(cursor.from, cursor.to);
      const compact = escapeInline(raw);
      const text =
        compact.length > maxLeafTextLength
          ? `${compact.slice(0, maxLeafTextLength)}…`
          : compact;
      line += `: "${text}"`;
    }
